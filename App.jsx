...
async function fbSet(key, value) {
  try { await setDoc(doc(db, "gastos_am", key), {value}); } catch(e) {}
}
...
function AnalisSection(gastos,anCat,setAnCat,anMes,setAnMes,anRango,setAnRango,fmt,fmtK,convVal,cl,CATEGORIAS,COLORES,MESES,mk,thS,tdS,btnSt,Cell) {
  ...
  var barAnD=rKsBar.map(function(k){var pts=k.split("-"),bm=parseInt(pts[1]),ba=parseInt(pts[0]);var tot=gCat.filter(function(g){return g.mes===bm&&g.anio===ba;}).reduce(function(s,g){var v=convVal(g.monto,k);return s+(v||0);},0);return{mes:MESES[bm-1]+"'"+String(ba).slice(2),total:tot,_mk:k};});
        ...
        <div>
          ...
          <div style={{background:"#fff",borderRadius:14,padding:"16px 8px 8px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)",height:260,marginBottom:8}}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barAnD} margin={{top:4,right:8,left:4,bottom:28}} onClick={function(e){if(e&&e.activePayload&&e.activePayload[0]){var k=e.activePayload[0].payload._mk;setAnMes(anMes===k?null:k);}}}>
                ...
                <Tooltip formatter={function(value){return["$"+Math.round(value).toLocaleString("es-AR"),"Total"];}} contentStyle={{fontSize:12,borderRadius:8,border:"1px solid #E2DDD6",background:"#fff",color:"#1a1a2e"}} cursor={{fill:"rgba(0,0,0,0.05)"}}/>
                          ...
                          {rows.map(function(r){
                            return (
                              <tr key={r.descripcion}>
                                ...
                                <td style={Object.assign({},tdS,{fontWeight:600,textAlign:"right"})}>{fmt(r.monto)}</td>
                              </tr>
                            );
                          })}
                          <tr style={{background:"#F5F3EF"}}>
                            <td style={Object.assign({},tdS,{fontWeight:700})} colSpan={2}>Total</td>
                            <td style={Object.assign({},tdS,{fontWeight:700,textAlign:"right"})}>{fmt(detalleMes.reduce(function(s,g){return s+g.monto;},0))}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
  ...
  return (
    <div className="gr">
      ...
      <div className="body">
        ...
        {vista==="graficos" && grafMounted && (
          <div>
            ...
            {cats.length===0 ? (
              ...
              var principales=catsPos.filter(function(e){return Math.abs(e[1])/Math.abs(totM)>=UMBRAL;});
              var otros=catsPos.filter(function(e){return Math.abs(e[1])/Math.abs(totM)<UMBRAL;});
              var otrosTotal=otros.reduce(function(s,e){return s+e[1];},0);
              ...
              // total formateado abreviado
              var totalAbs=Math.abs(totM);
              var totalLabel=(function(){
                if(mon==="USD"){if(!tcV)return"—";var v=totalAbs/tcV;return v>=1000?"U$S "+(v/1000).toFixed(1)+"k":"U$S "+Math.round(v);}
                var v=mon==="CER"?(fcer(fMes,fAnio)?totalAbs*fcer(fMes,fAnio):totalAbs):totalAbs;
                return v>=1000000?"$"+(v/1000000).toFixed(2)+"M":v>=1000?"$"+(v/1000).toFixed(1)+"k":"$"+Math.round(v);
              })();
              ...
              return (
                <div className="cw" style={{marginBottom:28,padding:"16px 0 16px"}}>
                  <div style={{position:"relative",width:"100%",height:300}}>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={pieData} cx="50%" cy="50%" innerRadius="38%" outerRadius="64%" paddingAngle={2} dataKey="value" labelLine={false} label={renderLabel}>
                    ...
                    <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",pointerEvents:"none"}}>
                      ...
                      <p style={{fontSize:16,fontWeight:700,color:"#1a1f3a",margin:0,lineHeight:1}}>{totalLabel}</p>
                    </div>
                  </div>
                  {/* Leyenda solo de categorias individuales, de mayor a menor */}
        ...
        {vista==="analisis" && anMounted && AnalisSection(gastos,anCat,setAnCat,anMes,setAnMes,anRango,setAnRango,fmt,fmtK,convVal,cl,CATEGORIAS,COLORES,MESES,mk,thS,tdS,btnSt,Cell)}
