...
function AnalisSection(gastos,anCat,setAnCat,anMes,setAnMes,anRango,setAnRango,fmt,fmtK,convVal,cl,CATEGORIAS,COLORES,MESES,mk,thS,tdS,btnSt,Cell) {
  ...
  var barAnD=rKsBar.map(function(k,idx){
    ...
    var maWindow=allMksCat.slice(Math.max(0,idxAll-11),idxAll+1);
    ...
    var ma=maWindow.length>=3?Math.round(maVals.reduce(function(s,v){return s+v;},0)/maVals.length):null;
      ...
      {gCat.length===0 ? (
        ...
        <div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={barAnD} margin={{top:4,right:8,left:4,bottom:28}} onClick={function(e){if(e&&e.activePayload&&e.activePayload[0]){var k=e.activePayload[0].payload._mk;setAnMes(anMes===k?null:k);}}}>
                ...
                <Tooltip formatter={function(value,name){
                  if(name==="ma") return ["$"+Math.round(value).toLocaleString("es-AR"),"MM 12m"];
          ...
          <div style={{display:"flex",gap:16,justifyContent:"center",marginBottom:16,fontSize:11,color:"#6B7280"}}>
            ...
            <span style={{display:"inline-flex",alignItems:"center",gap:5}}><span style={{width:16,height:2,background:"#C49A6C",display:"inline-block",borderTop:"2px dashed #C49A6C"}}/> Media móvil 12m</span>
          </div>
